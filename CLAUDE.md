...
- **DevApiStatus Events tab** — `src/components/ui/DevApiStatus.tsx`. Gated on `schemaVersion === 'v2' && import.meta.env.DEV`. Eight blocks: pipeline waterfall (D-16), precision+confidence+source-tier+casualty histograms via ProgressBar (D-17), per-event drill-down last 50 with copy-prompt button (D-18), LLM call log last 20 with amber `⊘` skip-entry glyph (D-19), per-provider budget bars with pause badge (D-36), eval score summary with D-25 PASS/FAIL gate (D-20), DLQ recent list (D-30), suspect count badge (D-23).
- **Topbar pipeline pill** — `src/components/layout/Topbar.tsx` `PipelineVersionPill` next to `DevApiStatusTrigger`. Dev-only. Shows `v2` green / `v1` amber. Click POSTs swap. Lazy-fetches on mount.
- **Phase 27.4 known P0 follow-up** — v2 extractor writes `events:llm:v2` cache only AFTER all batches complete. One hung Cerebras call loses 45+ minutes of LLM work (stall reproduced at batch 133/184 on 2026-04-21). Needs per-batch `cacheSetSafe` + timeout watchdog in `server/lib/llmEventExtractor.v2.ts` main loop. See `.planning/phases/27.4-llm-enrichment-improvements/27.4-HANDOFF.md` for full resume options.
