...
- **`cron:watch:v2.0` (Phase 46 CRON-WATCH-01 D-07)** — bounded LPUSH+LTRIM 14-cap / ~30d TTL once-daily cron-stability watch ring (same family as `dashboard:trends:history`). `WatchSample` row = `{sampledAt, tickDate, cronAgeMs:{health,warm,'refresh-events':number|null}, eval:{at5km,at20km,at100km}, dlqCount, breakerTrips, result:'PASS'|'FAIL'}`. Writer: `server/routes/cron-health.ts` via `server/lib/cronWatch.ts appendWatchSample` — appended AFTER the `cron:lastTick:health` + trend-sample writes in its OWN try/catch (degrade-open; a watch-write failure NEVER degrades the health response, Pitfall 4). **Piggybacks the EXISTING `0 0 * * *` health cron — no new cron, no new endpoint.** Reader: `readWatchHistory` + the `.planning/phases/46-general-hardening-cron-watch-start/46-WATCH.md` human-readable mirror. NON-BLOCKING CRON-WATCH-01 7-day watch — does NOT gate phase or milestone close (early close requires an explicit operator decision citing the v1.5 Phase 31 precedent).
...
- **`ratelimit:429:{tier}:{YYYY-MM-DD}` (Phase 46 HARD-01 D-02)** — INCR counter; per-tier per-UTC-day count of rejected (429) requests. 48h TTL, EXPIRE-on-first (only the first INCR of the day sets the TTL; second-and-later INCRs do NOT re-issue EXPIRE — self-rolls daily). `tier ∈ {flights, ships, events, news, markets, weather, sites, sources, geocode, water, public}` (the `rateLimiters` table keys). Writer: `server/middleware/rateLimit.ts` `incr429` — fired fire-and-forget (`void incr429(tier)`) from the 429 branch, fully try/caught so a Redis throw can NEVER convert a 429 into a 500 (degrade-open, T-46-01-01). Reader: `server/routes/operator-status.ts` `rateLimiter` block (sums today + yesterday for a rolling "recent 429s" view, surfaced alongside the `RATE_LIMITER_CONFIG` per-tier limits). Both key components are server-derived (tier name + UTC date) — no client-controlled input.
