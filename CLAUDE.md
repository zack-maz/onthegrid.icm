...
- **Cron schedule (Hobby cap 3 entries)** — `/api/cron/health 0 0 * * *` (Redis ping + freshness + eval-drift via `runEval()` + adversarial eval), `/api/cron/warm 0 12 * * *` (Overpass sites + water pre-warm), `/api/cron/refresh-events 0 4 * * *` (LLM v3 extraction).
...
For Phase 29 (LLM provider chain narrowing, LLM-optional architecture, Vercel Pro upgrade, domain rename), see [docs/adr/ADR-0010-llm-pipeline-v1-5-decisions.md](docs/adr/ADR-0010-llm-pipeline-v1-5-decisions.md) and [docs/runbook.md](docs/runbook.md).
