...
**v2.0 progress:** Phase 42 (Water Filter Fix) complete 2026-06-10 — telemetry-diagnosed and fixed the name-blind, order-dependent spatial-dedup loop (exported pure `spatialDedup()`, name-aware + deterministic survivor), bumped `water:facilities:v3` → `v4` across all 10 lockstep surfaces, regenerated the cold-start snapshot (304 → 460 admitted facilities). WATER-FILTER-01..04 validated (8/8 must-haves verified). Phase 43 (Ghost Link Prune Correctness) complete 2026-06-10 — 7-status liveness taxonomy (`+soft-404 +no-url`) with required-nullable `evidence` string; soft-404 body heuristic (capped 16 KiB GET on 200s, precision-first markers/redirect-home/near-empty); source-less events explicitly classified `no-url`; attemptCount semantics fixed (live resets, unknown preserves; unknown TTL 1h→24h so accumulation survives the daily cadence); evidence-gated 403 demotion — production sample showed 20/20 prod 403-URLs live under a browser UA, so `403` is excluded from cron auto-prune (manual prune + dashboard count unchanged); SC-3 audit FLAGGED 8 live URLs swept by the pre-fix prune (root cause = the demoted 403 path). GHOST-06..10 validated (5/5 truths verified; 3 Critical + 5 Warning code-review findings fixed in-phase). Phase 44 (Events Subtab Pipeline Detail) complete 2026-06-21 — LLM pipeline run→call→detail blocks wired into the Events subtab + a dead-URL sidecar count-drift hotfix (`reconcileDeadUrlCount`); shipped via PR #43. Phase 45 (Dashboard Subtab Readability Redesign) complete 2026-06-22 — three operator subtabs re-gridded for scannability (tabular-nums, progressive disclosure, two-weight hierarchy) behind a hard WAI-ARIA behavioral freeze, plus four trend sparklines fed by a new server-backed Redis history ring; DASH-READ-01..05 validated (13/13 must-haves; 0 blocker / 3 advisory-warning code review).
...
- ✓ Operator-dashboard subtab readability redesign: extracted `MetricRow` (tabular-nums right-aligned row) + `Sparkline` (~30-pt inline-SVG, neutral stroke + semantic last-point tint) atoms; Water/Sites/Events subtabs re-gridded with labeled Reason|Count tables, progressive-disclosure drill-downs (`DisclosureSection`), and a two-weight hierarchy (47 `font-bold`→`font-semibold`); four trend sparklines (3 cron-freshness + 1 dead-link) fed by a server-backed bounded Redis ring (`dashboard:trends:history`, LPUSH+LTRIM 30-cap/30d, written by the existing health cron, surfaced on `/api/operator-status` with route-test + OpenAPI + client-interface lockstep); WAI-ARIA tablist contract frozen byte-stable (4 behavioral pin suites zero-diff, snapshot zero-diff) — v2.0 (Phase 45, DASH-READ-01..05)
...
_Last updated: 2026-06-22 after Phase 45 (Dashboard Subtab Readability Redesign) completion. Six target features locked in operator priority order (water filter / ghost links + events subtab / dashboard subtab cleanup / ~100-user load test / general hardening / docs cleanup); Phases 42–45 of v2.0 complete, Phase 46 (General Hardening + Cron Watch Start) next. v1.6 archives at `.planning/milestones/v1.6-*`._
