...
- [x] **LLM-RELI-07**: `prod-connectivity-audit.yml` exit-0 with `audit:connectivity:last-result.allTiersGreen === true` for **3 consecutive runs** (the v1.5 → v1.6 promotion gate; unblocks 999.5 load test).
- [x] **LLM-RELI-08**: ~~Cerebras + Groq free-tier probe artifact committed~~ → CLOSED 2026-05-23 as `cerebras-groq-deferred` (Phase 34 honest deferral). No probe ran; operator chose to skip provisioning free-tier accounts. Decision recorded in ADR-0010 Phase 34 sub-block + `docs/architecture/llm-pipeline-reliability.md` "Multi-Provider Cascade (Phase 34)" section. Mirrors Phase 30.1's `nim-only` precedent — empirical "no provider expansion right now" is a load-bearing outcome.
- [x] **LLM-RELI-09**: ~~Adapter implementation for each passing provider~~ → CLOSED 2026-05-23 as `cerebras-groq-deferred`. No adapter code lands; `FreeProvider` union remains `'nvidia_nim' | 'openrouter'`. The "both probes fail → cerebras-groq-deferred status" close-out path baked into the requirement was triggered by operator choice rather than empirical probe failure — operationally equivalent outcome.
- [x] **LLM-RELI-10**: ~~Per-provider eval scoring~~ → CLOSED 2026-05-23 as deferred alongside LLM-RELI-08/09. No `providerProvenance` field, no `EvalScore.byProvider` shape, no UI block. Future provider-restoration phase would re-introduce alongside the adapter work.
- [x] **LLM-RELI-11**: ~~DLQ count drop validation~~ → CLOSED 2026-05-23 as deferred. No `v3:cascade_exhausted` DLQ reason added; no validation cron run. Phase 31 Day-1 baseline (4 × `v3:timeout_watchdog`) remains a known failure mode under the single-provider cascade — see ADR-0010 Phase 34 sub-block's "Phase-35-or-later follow-up candidates" list for mitigation paths (paid provider tier, adaptive Retry-After NIM limiter, etc.).
...
- [x] **ACTOR-01**: One-time audit of the live `events:llm:v3` snapshot focused on actor metadata. Categorizes failures into buckets: (a) actor field null/empty, (b) actor field carries a raw CAMEO actor code instead of a human-readable label, (c) actor field carries an ambiguous generic string (e.g. "soldiers", "the army", "forces"), (d) actor field disagrees with the linked source content. Report quantifies each bucket and lists representative examples. Output drives ACTOR-02 catalog seeding and ACTOR-04 eval rubric design. **(Audit tooling shipped Plan 33-01: classifier + audit script + AUDIT-REPORT.md stub. Report population pending operator UAT against staging Redis — see 33-VERIFICATION.md Manual-UAT.)**
- [x] **ACTOR-02**: Canonical actor catalog defined and committed (e.g. `server/data/actor-catalog.ts` or similar). Maps CAMEO actor codes + common abbreviations + observed LLM-extracted variants to canonical human-readable names (e.g. `IDF` → `Israeli Defense Forces`, `IRGC` → `Islamic Revolutionary Guard Corps`, `USMIL` → `United States Military`). Catalog scoped to Iran-conflict-relevant actors; out-of-scope actors fall through to whatever the extractor returned. Catalog has a contract test asserting no duplicate canonical names + no orphan codes. **(Plan 33-02: 27 entries; D-04..D-07 invariants enforced by `src/__tests__/lib/actorCatalog.test.ts`; 24/24 contract tests GREEN.)**
- [x] **ACTOR-03**: v3 LLM extractor prompt extended to emit canonical actor names where possible. Raw GDELT actor codes are mapped through the catalog before being written to `events:llm:v3`. New `actorConfidence` field added to the v3 schema (per-actor: high / medium / low). Schema migration handled via the existing v2/v3 cache-version discipline (i.e. `events:llm:v3` may bump to `events:llm:v3.1` if needed; or stay at v3 if the field is added forward-compat with a default). **(Plans 33-03 + 33-04: actorConfidence added as `.optional()` to `enrichedEventV3` for rollout safety; EVENT_EXTRACTION_SCHEMA_V3 un-aliased + JSDoc-pinned; `applyCatalogToEvents` + `repairActorConfidence` server-side post-validate; SYSTEM_PROMPT_V3 extended; 175/175 LLM-pipeline tests GREEN.)**
- [x] **ACTOR-04**: Eval harness ground-truth set (`.planning/eval/ground-truth-events.json`) extended with `expectedActor1` + `expectedActor2` fields per event. Daily eval scores actor-match rate alongside the existing geocode accuracy at 5 / 20 / 100 km thresholds. Adversarial fixtures (`.planning/eval/adversarial-injections.json`) extended with one or more actor-confusion injections (e.g. prompt designed to swap actor sides). **(Plan 33-05 + gap-fix `ab6ad0d`: 50/50 GT events backfilled; 3 adversarial entries adv-011..013; `runEval()` substring landmark+country join per D-13; `actorMatchRate` rendered in EvalScoreBlock alongside D-25 gate.)**
- [x] **ACTOR-05**: API Health dashboard surfaces actor-quality counts — events with null actors, raw-CAMEO actors, ambiguous-string actors, low-confidence actors. Drill-down list available via the existing per-event detail panel; counts feed into the `aggregateHealth.endpoints.llmEvents` quality metrics block surfaced in Phase 28.2 W5 D-23. **(Plans 33-06 server-side + 33-07 client-side: `/api/operator-status` returns `actorQuality` block; DevApiStatus renders 4 counters + drill-down sample list capped at 20; degrade-open on Redis fail; 6/6 RTL tests + 6/6 supertest integration tests GREEN.)**
