...
updated: 2026-06-06T19:20:00Z
resolution: both-fixes-shipped-and-verified-in-prod (primary env + secondary 48h-TTL hotfix PR#38)
---

> **PRODUCTION RESOLUTION (2026-06-06 ~19:18Z) — both fixes live + verified.**
>
> - PRIMARY (env): removed prod `CACHE_KEY_PREFIX`, redeployed → prod writes/reads the unprefixed namespace.
> - SECONDARY (TTL): cherry-picked source onto a hotfix branch off main → **PR #38 merged to main (4dd281e)** → Vercel auto-deployed prod (`j4jgaq0k1`, built from 4dd281e). `api/vercel-entry.js` NOT hand-committed on the hotfix — Vercel rebuilds it from source via `npm run build` (confirmed in build logs).
> - VERIFIED LIVE: force-triggered the cron on the new deployment → fresh run enriched 78 events (eval within20km 49/50, error=null, ~125s) and wrote `events:llm:v3` with **TTL=172785s (48h)** — up from the old ~7000s; `/api/events` = 182/182 enriched, `/api/health` llmEvents healthy.
> - CI note: the hotfix PR's `test` check was red on a PRE-EXISTING, unrelated failure — `src/__tests__/lib/actorCatalog.test.ts` reads `.planning/phases/33-.../cameo-codes.json`, a fixture committed on NO branch (main's `test` was already red). The hotfix's own tests + tsc + lint-and-typecheck were green.
> - CLEANUP: DEL'd the stale unprefixed `events:llm-pipeline-audit` (3 entries, newest 2026-04-29) so the dashboard "Pipeline Flips" block stops showing residue from the Phase-29-deleted v1/v2/v3 mechanism. Left short-TTL `dev:*` keys to expire on their own.
> - OPEN FOLLOW-UPS: (a) the broken actorCatalog.test.ts fixture (main CI red); (b) optional local `.env.local` trailing-space typo (`dev: ` → `dev:`); (c) the dashboard surfacing the dead `events:llm-pipeline-audit` key is a cosmetic reporting bug worth a code guard so it can't recur; (d) this branch's commit 7140373 duplicates the now-merged hotfix — expect a trivial auto-merge when v1.6 lands on main.
...
confirming_evidence: - "Direct prod-Upstash probe: every UNPREFIXED operational key (events:llm:v3, events:gdelt, cron:lastTick:*, news:feed, flights:_, ships:ais, markets:_) is absent (TYPE=none / EXISTS=0), while the `dev: `-prefixed equivalents are LIVE and fresh (dev: flights:adsblol TTL=278s, dev: ships:ais TTL=279s, dev: events:gdelt TTL≈2.4h)." - "The three `dev: cron:lastTick:_`keys decode to exactly 00:00:11Z / 04:00:42Z / 12:02:49Z UTC — precisely the three vercel.json cron schedules (0 0 \* \* _, 0 4 _ \* _, 0 12 _ \* \*). Only the Vercel PRODUCTION scheduler fires at those exact times, so the deployed app (not a local dev process) is writing the`dev: `prefix." - "dev: events:llm-summary:v3 proves a fully successful enrichment run today (enrichedCount=72, error=null, eval within20km 49/50) — pipeline works; only the namespace is wrong." - "dev: events:llm:v3 is TYPE=none/TTL=-2 at 18:17Z despite the 04:02Z successful write — LLM*REDIS_TTL_SEC=9000 (2.5h) expires it ~06:32Z, and nothing rewrites it until the next 04:00 cron (~21.5h gap)." - "Contract evidence: config.ts L160-168 + .env.example L188-192 state production must NOT set CACHE_KEY_PREFIX; the unprefixed audit:connectivity:last-result (written by the prefix-less GitHub Action) is green, confirming unprefixed is the intended prod home."
