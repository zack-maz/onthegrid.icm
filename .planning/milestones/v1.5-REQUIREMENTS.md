# Requirements Archive: v1.5 LLM Reliability & Reveal Prep

**Archived:** 2026-06-03
**Status:** SHIPPED

For current requirements, see `.planning/REQUIREMENTS.md` (created fresh for the next milestone via `/gsd:new-milestone`).

**v1.5 outcome summary:** 47 requirements / 47 mapped to phases / all closed at milestone shut-down:

- 43 closed `Complete` against shipped code.
- 1 closed `Validated single-day` (LLM-RELI-06) — Phase 31 closed early at Day 1 / 7 under operator decision; the 7-consecutive bar was never met. Snapshot harness operational. Caveat preserved in Phase 31 SUMMARY.md.
- 4 closed `cerebras-groq-deferred` (LLM-RELI-08..11) — Phase 34 operator deferral. No probe, no adapter, no per-provider eval, no `v3:cascade_exhausted` DLQ reason. Mirrors Phase 30.1 `nim-only` precedent. Planning artifacts preserved for any future provider-restoration phase.

LLM-RELI-07 (3 consecutive `prod-connectivity-audit.yml` greens) is the milestone-close gate — satisfied 2026-06-03 via 4 architectural unblocker PRs landed during observation (PR #32 `llmEvents` demoted to non-critical; PR #33 `news` GDELT-DOC best-effort; PR #34 D-03 truth-table relaxed for non-critical tier; PR #35 YAML/shell apostrophe-quoting hotfix). v1.6 promotion unblocked.
