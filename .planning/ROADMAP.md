...
- [ ] **Phase 36: ADR-0010 + Acceptance Gate Closeout** — Capture v1.5 LLM-pipeline decisions in a new ADR (ADR-0010 — the 0009 slot is taken by the existing Accepted two-key-split ADR); observe `prod-connectivity-audit.yml` exit-0 with `allTiersGreen=true` for 3 consecutive runs; lock the milestone close.
...
**Requirements**: LLM-RELI-01, LLM-RELI-05, SIMPLIFY-04, SIMPLIFY-06 (folded forward from Phase 34 on 2026-05-09 — full deletion not archive), DOCS-INT-01 (pulled from Phase 34 on 2026-05-09)
...
**Plans:** 4 plans (Wave 1: 30.1-01 probe; Wave 2: exactly ONE of 30.1-02 Right scope / 30.1-03 middle bucket / 30.1-04 Minimum scope, selected at probe checkpoint per D-05 thresholds)

Plans:

- [ ] 30.1-01-PLAN.md — Probe OpenRouter free-tier rate-limit behavior + capture snapshot + operator selects Wave 2 plan (D-03..D-06)
- [ ] 30.1-02-PLAN.md — Right scope (< 50% probe fail): remove `skipOpenRouter: true` from both v3 call sites + rewrite P3 test + force-trigger validation cron + amend docs (D-01, D-07, D-10..D-16) — runs ONLY if probe lands `restored-cascade`
- [ ] 30.1-03-PLAN.md — Middle bucket (50-90% probe fail): docs-only amendment with measured percentages + Phase 31 paid-OR deferral note (D-01, D-08, D-10..D-13) — runs ONLY if probe lands `middle-bucket-defer`
- [ ] 30.1-04-PLAN.md — Minimum scope (> 90% probe fail): docs-only amendment declaring NIM-only active, OR dormant (D-01, D-08, D-10..D-13) — runs ONLY if probe lands `nim-only`
...
### Phase 36: ADR-0010 + Acceptance Gate Closeout
...
**Depends on**: Phase 29 + 30 + 31 (LLM-RELI track must have shipped — ADRs document already-made decisions, not aspirational ones; and the gate observes the narrowed cascade running stably). Can be informed by Phase 35's public-docs work but ADR-0010 stands alone as the gate-close artifact.
