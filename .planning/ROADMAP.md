...
- [ ] **Phase 34: LLM Router Fallback Re-integration (Cerebras / Groq + Per-Provider Eval)** — Probe-then-restore Cerebras + Groq free-tier adapters in `freeClaudeRouter.ts` so NIM throttle events stop translating into DLQ entries (4 × `v3:timeout_watchdog` baseline observed Phase 31 Day-1). Add per-provider eval scoring so any restored provider silently producing worse extractions surfaces immediately. Probe-driven decision gate: if free-tier throttles are correlated with NIM's, honest close-out matches Phase 30.1's `nim-only` precedent. _(Inserted 2026-05-19 on operator request — primary lever is reliability / fallback under throttle; per-provider eval is the bounded quality lever paired with multi-model output.)_
...
- [ ] 34-01-PLAN.md — Probe Cerebras + Groq free tiers via `scripts/probe-cerebras-groq.ts` mirroring `probe-openrouter.ts`. Output snapshot. Per-provider pass/fail decision.
- [ ] 34-02-PLAN.md — Adapter restoration. For each passing provider: extend `FreeProvider` union; add provider config; add to cascade builder; independent circuit breaker + token-budget keys. Atomic per-provider commits.
- [ ] 34-03-PLAN.md — Per-provider eval instrumentation. Extend `llmEvalHarness.ts` + `LLMRunSummary` + API Health dashboard block. Eval-replay sanity check.
- [ ] 34-04-PLAN.md — Validation cron run. Verify `callHistory` shows non-NIM providers; per-provider eval within 3pp tolerance; DLQ count drop. Snapshot harness captures result row.
- [ ] 34-05-PLAN.md — Phase close — SUMMARY.md, ROADMAP/REQ/STATE flips, ADR-0010 supplement (or new ADR-0011 if scope warrants) recording providers reinstated + the probe-derived rationale.
...
| 33. Actor Metadata Audit, Canonical Catalog & Eval Expansion                  | 5/7                  | In Progress           |            |
