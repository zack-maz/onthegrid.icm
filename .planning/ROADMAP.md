...
8. **v1 + v2 extractor modules deleted** (folded from SIMPLIFY-06; supersedes Phase 27.4 D-26/D-40 deep-rollback lock). `server/lib/llmEventExtractor.v1.ts` + `server/lib/llmEventExtractor.v2.ts` removed from the codebase. Override endpoint `POST /api/events/llm-pipeline` route + `events:llm-pipeline-override` Redis key + `refreshPipelineOverride()` helper + `setPipelineOverride()` removed. DevApiStatus Pin-to-v1/v2 buttons + confirm-modal removed (Phase 28.2 W5 D-22 surface). Rationale captured in ADR-0009 stub written this phase (full ADR completed at Phase 36): v1→v2→v3 evolution, v3 cascade has been live since Phase 27.4 (~2026-04-21), Pitfall 1 cache bridge handles "map never goes blank" independently of which extractor is active, rollback path is now `git revert` of the Phase 29 commit range.
   **Plans**: TBD
