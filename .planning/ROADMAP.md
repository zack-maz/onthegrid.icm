...
- [ ] 39-01-PLAN.md — Flight-recorder data layer: RunHistoryEntry/CallHistoryEntry types + runId field; llm:calls:history + llm:runs:history Redis modules; 3-surface registry registration (HARD gate) [OBS-FLIGHT-01, -02, -06]
- [ ] 39-03-PLAN.md — tokenBudget degrade-open block on /api/operator-status + Zod .strict() contract test + exported ratio constants [BUDGET-03, -04]
...
- [ ] 39-04-PLAN.md — Bearer-gated GET /api/events/llm-history ({runs,calls} + ?runId/?limit) + cold-start hydration hooks + route test [OBS-FLIGHT-03, -06]
...
| 39    | Operator Visibility — Budget + Cost + LLM Flight Recorder             | 0/5            | Planned     | —          |
