...
decisions:
  ...
  - 'V1+V2 dev file cache write paths consolidated to saveDevLLMCacheV2: the v1 path used `saveDevLLMCache` (a separate function targeting events:llm.json on disk). After Plan-06 only the v3 pipeline writes dev files and it uses the v2 helper name (saveDevLLMCacheV2 / loadDevLLMCacheV2). The misleading-name issue is documented in CONTEXT D-02 / A7 as a Phase 34 rename target.'
metrics:
  tasks_completed: 7
  files_modified: 13
  files_deleted: 2
...
- **`saveDevLLMCacheV2` / `loadDevLLMCacheV2` misleading names.** Per CONTEXT D-02 / RESEARCH.md A7, the v2-suffixed devFileCache helpers are used by the v3 pipeline too. Rename deferred to Phase 34 (touches the dev-fixture file format on disk; not a Phase 29 concern).
