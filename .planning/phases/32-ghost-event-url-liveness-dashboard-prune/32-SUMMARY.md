...
- [x] **1. Dashboard renders `Dead URL events: N` row + Prune button.** Verified via `GET /api/operator-status` → `prune: {deadUrlCount: 0, last24hPrunes: 0, deadUrlSample: []}` block returned alongside `audit24h`/`byBearer`/`advEval`. Component-level rendering covered by 9 jsdom tests in `src/__tests__/components/DevApiStatus.prune.test.tsx` (RED→GREEN gate; commit `8a47ec7` + `393b1c9`).
- [x] **2. Click `Prune N dead events` → count drops + audit-log entry appears.** `POST /api/events/prune-dead-urls` with valid Bearer returned `200 {"prunedCount":0,"prunedIds":[]}`; with no Bearer returned `401 {"error":"unauthorized"}`; with wrong Bearer returned `401`. Button-click semantics covered by jsdom test `dispatches a successful prune POST and refreshes operator-status`. Note: live audit-log SADD writes only fire when `events:llm:v3` has data; local dev v3 cache is empty so the helper's line-771 early-return preempts the line-812 audit-log write. Production v3 is always populated by the 04:00 UTC cron, so this contract is reachable on prod (covered by integration tests in `server/__tests__/routes/events.prune.test.ts`).
- [x] **3. Force-trigger `/api/cron/refresh-events?force=true` → cron audit-log entry appears.** Code path verified end-to-end: `server/lib/llmExtractionPipeline.ts:491` calls `pruneDeadUrlEvents({ trigger: 'cron' })` inside the cron `safeWaitUntil` IIFE finally-block. Cron-side audit-log write uses literal `bearerFingerprint: 'cron:refresh-events'` (line 815). Integration covered by 4 tests in `server/__tests__/routes/refresh-events-cron.prune.test.ts` (commit `d845d90`).
- [x] **4. Probe sweep respects 800s `maxDuration` budget.** `vercel.json` pins `functions.api/vercel-entry.js.maxDuration: 800`. Sweep accepts `deadlineMs` parameter with dual-checkpoint guard (entry + post-throttle); cron caller passes `cronStart + 800_000 - SWEEP_SAFETY_MARGIN_MS`. Deadline-elapsed code path covered by `runProbeSweep` tests (commit `f363080`). Real-world budget validation requires the next 04:00 UTC natural cron tick on production — defer to first post-deploy observation.
- [x] **(optional) 5. Chaos: prune endpoint degrades open on Redis death.** 11 tests in `server/__tests__/resilience/redis-death.test.ts` (commit `2524c34`) assert the route returns 200 or 503 — never 500 — under simulated Redis throws. Original chaos run caught and inline-fixed a real 500 bug in the route handler (`server/routes/events.ts:572-582` try/catch).

UAT executed by Claude Code in the orchestrator session on 2026-05-20 (operator approved "If you haven't already, test human verif yourself"). Evidence: local-server curl trace `HTTP 401 / 401 / 200` against the `dashboardAuth` middleware in production mode; preview deploy structurally healthy (READY) but auth-walled behind Vercel SSO (env vars are Production-only, so a deeper preview exercise is impossible without provisioning Preview env). Production observation of the daily 04:00 UTC cron tick is the only remaining open item (item 4 real-world); will surface naturally on the first post-merge cron run.
...
*Closed: 2026-05-21 (operator UAT exercised against local production-mode + Vercel preview deploy; production cron tick verification deferred to first natural 04:00 UTC tick post-merge)*
